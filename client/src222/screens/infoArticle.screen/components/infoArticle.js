...
class InfoArticle extends Component {
  ...
  addNewChat = async (ownerId, buyerId, articleId) => {
    ...
    if (articleChat.length) {
      ...
      addChat({ ownerId, buyerId, articleId })
        .then((res) => {
          ...
          navigation.navigate('Messages', { name: user.username, id: res.data.addChat.id, loadingMoreEntries: false }); // Then recupera la respuesta de la mutacion
        });
    }
  }
...

          {
            article.owner.id === 1
              ...
              : (
                <View style={styles.buttons}>
                  <TouchableHighlight
                    onPress={() => this.addNewChat(article.owner.id, 1, article.id)}
                    style={styles.button}
                    underlayColor="transparent"
                  >
